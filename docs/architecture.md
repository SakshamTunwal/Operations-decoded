...
Design the module visual language you actually want — the framer-motion scene vocabulary, the character treatment, the progress and scorecard design. Apply it to the engine, so every future module inherits it. **Also in this phase:** write `content/company.json` once and deeply — the company (name, city, industry, background) and 6–8 recurring characters with names, roles, departments, personalities and speech habits. Every module draws its cast from here; avatars are generated once.
**Done when:** the ported module looks like the product you want to sell, and the company bible validates against `CompanySchema`.
