...
**Changelog v1.7 (2026-09-13):** Density pass shipped, per the Gold Standard ledger. Rooms v2: ceiling lights with glow cones, wall paneling, floor planks, depth layers (back-row racks), new props (box stacks, pallet jack, bookshelf, wall clocks, dock bumpers/cones, filing drawers), desaturated ambient background workers. Sprites v2: necks, shoulders, hands, shoes, and a raised-arm gesture while talking. **In-scene decision panel:** decision steps now play inside the scene (dark panel, A/B/C cards, implication lines, ✓/✗ verdicts); below-stage area shows feedback + teaching point after the pick. **Live scene data:** new additive schema field `journey.sceneData` {label, value?, tone?} renders as a DataPlaque inside the world and changes step to step — the sample now narrates itself through plaques (8 DAYS LEFT → CHECK IT → 4,500 KG → 4,700 KG? → PAID · $8,325). Standard ledger updated: 4 of 5 next-up assets ✅; drag-to-order remains to close Phase 2.

**Changelog v1.6 (2026-09-13):** The **Module Gold Standard** written — `docs/module-standard.md`, the benchmark that binds humans, agents and CI: story architecture (spine, 7-step arc, dollar-stated stakes, calendar-proof timelines), world requirements (≥2 motivated locations, live in-scene data labels, one scene event per module), interaction bar (≥4 interactive steps / ≥3 types, wrong answers teach a cost, decisions in-scene), writing bar (swap test, ≤2-sentence bubbles, ESL rules, 1-gag budget), motion bar, CI-enforced budgets (Lighthouse 85/90/95, 390px), the one-time asset ledger with status, and the shipping acceptance checklist. The upgraded sample module becomes the living benchmark; the M15 pilot must match or beat it. od-architect's SOUL and od-verifier's checklist will be derived from this document in Phase 3/4.
