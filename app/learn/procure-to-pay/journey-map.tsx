...
interface JourneyMapProps {
  currentStep: number;
  tiO2Days: number | null;
}

export function JourneyMap({ currentStep, tiO2Days }: JourneyMapProps) {
    ...
    <div className="flex items-center gap-3 min-w-0">
      ...
      <AnimatePresence mode="wait">
        {tiO2Days !== null && (
          ...
          >
            <motion.span
              animate={tiO2Days === 1 ? { opacity: [1, 0, 1] } : { opacity: 1 }}
              transition={{ duration: 1, repeat: tiO2Days === 1 ? Infinity : 0 }}
              className={`w-1.5 h-1.5 rounded-full ${
                tiO2Days <= 0 ? "bg-green-500" : tiO2Days === 1 ? "bg-red-500" : "bg-[#f59e0b]"
              }`}
            />
            {tiO2Days <= 0 ? "Delivery arrived! ✓" : `TiO₂: ${tiO2Days} day${tiO2Days !== 1 ? "s" : ""} left`}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
