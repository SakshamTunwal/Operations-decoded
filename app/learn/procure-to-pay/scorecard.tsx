...
import { VENDORS, STEPS, fmt } from "./constants";
import type { GameState } from "./types";
import { MarcusAvatar, DianaAvatar, LeonAvatar } from "./characters";
...
function calcScore(gs: GameState): { score: number; grade: string; color: string } {
  let s = 0;
  // Quantity: 2000 optimal
  if (gs.chosenQuantity === 2000) s += 25;
  else if (gs.chosenQuantity === 2500) s += 15;
  else s += 5;
  // Urgency: urgent optimal
  if (gs.chosenUrgency === "urgent") s += 20;
  else if (gs.chosenUrgency === "emergency") s += 15;
  else s += 5;
  // Vendor: crestline = 20pts, chembridge = 15, primechem = 5
  if (gs.selectedVendorIdx === 0) s += 20;
  else if (gs.selectedVendorIdx === 2) s += 15;
  else s += 5;
  // PO errors
  s += gs.poErrorsFound * 8;
  // Criteria ranking (if reliability is first → +7)
  if (gs.criteriaRanking[0] === "reliability") s += 7;
  ...
  const items = [
    {
      ...
      pts: gs.chosenQuantity === 2000 ? 25 : gs.chosenQuantity === 2500 ? 15 : 5,
      max: 25,
      detail: gs.chosenQuantity === 2000 ? "Optimal buffer" : gs.chosenQuantity === 2500 ? "Slightly over-ordered" : "Risky low stock",
    },
    {
      label: "Urgency Level",
      pts: gs.chosenUrgency === "urgent" ? 20 : gs.chosenUrgency === "emergency" ? 15 : 5,
      max: 20,
      detail: gs.chosenUrgency === "urgent" ? "Matched the situation" : gs.chosenUrgency === "emergency" ? "Escalated unnecessarily" : "Too relaxed for the urgency",
    },
    {
      label: "Vendor Selection",
      pts: gs.selectedVendorIdx === 0 ? 20 : gs.selectedVendorIdx === 2 ? 15 : 5,
      max: 20,
      detail: vendorName,
    },
    {
      label: "PO Accuracy",
      pts: gs.poErrorsFound * 8,
      max: 24,
      detail: `${gs.poErrorsFound}/3 errors caught`,
    },
    {
      label: "Criteria Ranking",
      pts: gs.criteriaRanking[0] === "reliability" ? 7 : 0,
      max: 7,
      detail: gs.criteriaRanking[0] === "reliability" ? "Reliability first — correct" : "Reliability wasn't prioritised",
    },
  ];
...
export function DynamicScorecard({
  ...
  const total  = vendor.price * gameState.grnQuantityReceived;
  const cheapest = Math.min(...VENDORS.map((v) => v.price));
  const overpaid = (vendor.price - cheapest) * gameState.grnQuantityReceived;
    ...
    <div>
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-[#6B7280] text-sm">
          You ran a full Procure-to-Pay cycle for Nexara Industrial Coatings.
        </p>
      </div>
      ...
      <div className="flex items-end justify-center gap-6 mb-8">
        {[
          { Av: MarcusAvatar, name: "Marcus", title: "Production" },
          { Av: DianaAvatar,  name: "Diana",  title: "Procurement" },
          { Av: LeonAvatar,   name: "Leon",   title: "Crestline" },
        ].map(({ Av, name, title }, i) => (
          <motion.div
            key={name}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white border border-[#E8E4DD] rounded-2xl p-5 shadow-sm">
          ...
          <ScoreRow label="Total Value Processed" value={`$${fmt(total)}`} delay={0.1} />
          <ScoreRow label="Vendor Selected" value={vendor.name} subtext={`$${vendor.price}/kg`} delay={0.15} />
          <ScoreRow
            label="vs Cheapest Option"
            value={overpaid > 0 ? `+$${fmt(overpaid)}` : "Optimal"}
            subtext={overpaid > 0 ? `PrimeChem was $${cheapest}/kg` : undefined}
            color={overpaid > 0 ? "text-[#D97706]" : "text-green-600"}
            delay={0.2}
          />
          <ScoreRow
            label="PO Errors Caught"
            value={`${gameState.poErrorsFound}/3`}
            color={gameState.poErrorsFound === 3 ? "text-green-600" : gameState.poErrorsFound >= 2 ? "text-[#D97706]" : "text-red-500"}
            delay={0.25}
          />
          <ScoreRow
            label="GRN Quantity"
            value={`${fmt(gameState.grnQuantityReceived)} kg`}
            color={gameState.grnQuantityReceived === gameState.chosenQuantity ? "text-green-600" : "text-[#D97706]"}
            delay={0.3}
          />
          <ScoreRow label="Real-world Timeline" value="~37 business days" delay={0.35} />
          <ScoreRow label="People Involved" value="4" subtext="Marcus, Diana, Leon, Finance" delay={0.4} />
        </div>
        ...
        <div className="bg-white border border-[#E8E4DD] rounded-2xl p-5 shadow-sm flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-xs text-[#9CA3AF] text-center leading-relaxed">
            Based on quantity choice, vendor selection, urgency level, and PO accuracy.
          </p>
        </div>
      </div>
      ...
      <div className="border-l-4 border-[#f59e0b] bg-[#FFFDF5] rounded-r-2xl p-5 mb-8">
        <p className="text-[#4B5563] text-sm italic leading-relaxed">
          "This happens hundreds of times a month at any mid-sized company. Now you know what's actually happening."
        </p>
      </div>
