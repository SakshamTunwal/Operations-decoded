...
interface JourneyMapProps {
  currentStep: number;
  flashSaleDay: number | null;
}

export function JourneyMap({ currentStep, flashSaleDay }: JourneyMapProps) {
    ...
    <div className="flex items-center gap-3 min-w-0">
      ...
      <AnimatePresence mode="wait">
        {flashSaleDay !== null && (
          ...
          >
            <motion.span
              animate={flashSaleDay === 0 ? { opacity: [1, 0, 1] } : { opacity: 1 }}
              transition={{ duration: 1, repeat: flashSaleDay === 0 ? Infinity : 0 }}
              className={`w-1.5 h-1.5 rounded-full ${
                flashSaleDay >= 3 ? "bg-green-500" : flashSaleDay >= 1 ? "bg-[#f59e0b]" : "bg-red-500"
              }`}
            />
            {flashSaleDay === 0 ? "Flash Sale LIVE ⚡" : `${flashSaleDay} week${flashSaleDay !== 1 ? "s" : ""} to flash sale`}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
