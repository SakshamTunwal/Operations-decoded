...
export function JourneyMap({ currentStep, contractDays }: JourneyMapProps) {
  ...
  return (
    <div className="flex items-center gap-3 min-w-0">
      ...
      {/* Contract Renewal countdown pill */}
      <AnimatePresence mode="wait">
        {contractDays !== null && (
          ...
          >
            <motion.span
              animate={contractDays <= 10 && contractDays > 0 ? { opacity: [1, 0, 1] } : { opacity: 1 }}
              transition={{ duration: 1, repeat: contractDays <= 10 && contractDays > 0 ? Infinity : 0 }}
              className={`w-1.5 h-1.5 rounded-full ${
                contractDays <= 0 ? "bg-green-500" : contractDays <= 10 ? "bg-red-500" : "bg-[#f59e0b]"
              }`}
            />
            {contractDays <= 0 ? "Day 90 ✓" : `Contract: ${contractDays} days left`}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
