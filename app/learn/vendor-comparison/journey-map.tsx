...
interface JourneyMapProps {
  currentStep: number;
  palmOilDays: number | null;
}

export function JourneyMap({ currentStep, palmOilDays }: JourneyMapProps) {
    ...
    <div className="flex items-center gap-3 min-w-0">
      ...
      <AnimatePresence mode="wait">
        {palmOilDays !== null && (
          ...
          >
            <motion.span
              animate={palmOilDays <= 2 ? { opacity: [1, 0, 1] } : { opacity: 1 }}
              transition={{ duration: 1, repeat: palmOilDays <= 2 ? Infinity : 0 }}
              className={`w-1.5 h-1.5 rounded-full ${
                palmOilDays <= 0 ? "bg-green-500" : palmOilDays <= 2 ? "bg-red-500" : "bg-[#f59e0b]"
              }`}
            />
            {`Palm Oil: ${palmOilDays} day${palmOilDays !== 1 ? "s" : ""} left`}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
